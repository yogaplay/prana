...
(
    ...
    CONSTRAINT fk_tag FOREIGN KEY (tag_id) REFERENCES tag (tag_id),                     -- tag 테이블과 연결
    CONSTRAINT uk_sequence_tag UNIQUE (tag_id, sequence_id)                             -- 동일한 tag_id와 sequence_id의 조합은 중복되지 않도록 제약
);
